...
public class DiscretizationTest {
    ...
    /** ********************************************************************
     * Testing line starting at[0.0; 0.0] and ending at [1.0; 0.0]
    ...
    /** ******************************************************************** */
    public void test(){
        for (int i=2000; i<=numberOfSteps; i++){
//            result.put(i, _getLength(i));
            
            System.out.println("Length for " + i + " discretization: " + (_getLength(i)-1.0));
...
//        _calculateLength();
    ...
    private void _discretizeLine(int parts){
        ...
        for(int i=2; i<=parts; i++){
            next = previous+step;
            discretizationPoints.add(new Point(next, 0));
            previous=next;
        }
    ...
    private double _calculateLength(){
        ...
        for(int i=1; i<discretizationPoints.size(); i++){
            length += (discretizationPoints.get(i).getX() - prev.getX());
